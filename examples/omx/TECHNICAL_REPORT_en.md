...
**NEXT's load-detection resolution limit and pose-dependent bias (Phases 11–14)**: To improve external-force-estimation accuracy, we ran pose-binned `tau_ext` noise-floor bias diagnostics (`evaluate_pose_dependence.py`), static-pose A/B load-detection tests (`collect_static_hold.py`, later redesigned around teleoperated grasping and a pinned pose via `--output_loaded` to eliminate hand-contact contamination and pose-repeatability drift), a training-data q-space coverage checker (`check_training_coverage.py`), a switch to operator-driven free-motion collection targeting task-relevant poses (`collect_free_motion_teleop.py`), and several retraining cycles (see §4 Step 7). The result: **large load differences (~100g scale) are clearly detectable, and adding per-joint `tau_ext` as an ACT observation channel successfully learned to sort objects by weight**. However, even for the same 100g object, `tau_ext`'s variation with pose remained large. Across retraining cycles, improvements were pose-region-specific (better at one tested pose, worse at another) rather than a general gain. We attribute this to a **structural sensing-resolution limit** — the `Present_Load` register used by `shoulder_lift`/`elbow_flex` (XL430) is a coarse PWM-duty-derived estimate (§2.5), compounded by mechanical backlash — rather than a data-coverage or tuning deficiency, and accept it as a known limitation. Further tuning cycles (more data collection/retraining) chasing finer-grained load discrimination are not planned; large-load detection and the downstream tasks it enables (e.g. ACT-based weight sorting) are treated as the practical operating range.
