...
SAFETY -- read before running:
  ...
  2. Support the arm by hand before starting. `--modifier` defaults to a conservative 0.2;
     increase it gradually (Ctrl+C, rerun with a higher value) while feeling whether the arm
     gets lighter, rather than jumping to a large value.
...
Usage (run from repo root):
    python -m examples.omx.gravity_compensation.gravity_comp_demo \\
        ...
        --urdf_path /path/to/omx_l.urdf --modifier 0.2
"""
...
def main():
    ...
    parser.add_argument("--modifier", type=float, default=0.2, help="Gravity-comp gain, start low")
    ...
    try:
        ...
        print(
            f"Current Control Mode enabled (modifier={args.modifier}, "
            f"current_limit={args.current_limit_ma}mA). Ctrl+C to stop.\n"
        )
        while True:
            ...
            goal_current_ma = {}
            for joint in ARM_JOINTS:
                current_a = (tau_g[joint] / KT_NM_PER_A) * args.modifier
