...
**NEXTによる負荷検出の解像度限界とpose依存バイアス（Phase 11〜14）**: 外力推定精度の改善を目的に、姿勢ごとの`tau_ext`ノイズ床の偏り診断（`evaluate_pose_dependence.py`）、静止姿勢でのA/B負荷検出テスト（`collect_static_hold.py`。後に、人手の接触混入と姿勢再現性の問題を解消するためテレオペ把持・姿勢固定〔`--output_loaded`〕へ改良）、学習データのq空間カバレッジ確認（`check_training_coverage.py`）、実タスクに近い自由運動データの再収集（`collect_free_motion_teleop.py`）と複数回の再学習を行った（§4 Step 7 参照）。結果として、**約100g規模の負荷差は明確に検出可能であり、ACTの観測情報に各関節の`tau_ext`を追加することで重量による物体の仕分けが実際に学習・成功する**ことを確認した。一方で、同じ100gの物体でも姿勢によって`tau_ext`の変動幅が大きく異なることも判明した。再学習を繰り返しても改善は姿勢領域ごとにまちまちで（ある姿勢で改善する一方、別の姿勢では悪化する）、汎用的な精度向上には至らなかった。これは学習データのカバレッジやチューニング手法の問題というより、`shoulder_lift`/`elbow_flex`（XL430）が用いる `Present_Load` レジスタ（PWMデューティ比由来の粗い推定値、§2.5）とバックラッシュに起因する **構造的なセンシング解像度の限界** と結論づけ、既知の制約として受容している。今後、この方向（データ収集・再学習の繰り返し）でさらに微細な負荷弁別の精度を追求することはせず、大きな負荷差の検出とそれを用いた下流タスク（ACTでの重量仕分けなど）を実用範囲として扱う。
